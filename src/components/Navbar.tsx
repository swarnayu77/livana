      ...
      <div className="max-w-[1200px] mx-auto px-6 lg:px-8">
        <div className="flex items-center justify-between h-[60px]">
          ...
          <Link to="/" className="flex items-center gap-2.5 group">
            <img src={livanaLogo} alt="LIVANA" className="h-7 w-7 object-contain transition-transform duration-200 group-hover:scale-105" />
          ...
          <nav className="hidden md:flex items-center gap-1">
            {navLinks.map((link) => {
              ...
              return link.href.startsWith("/#") ? (
                <a key={link.label} href={link.href}>{inner}</a>
              ) : (
          ...
          <div className="hidden md:flex items-center gap-2">
            ...
            {user ? (
              ...
              <>
                <Link to="/auth">
                  <Button variant="ghost" size="sm" className="text-[13px] h-9 rounded-lg px-3.5">Sign In</Button>
                </Link>
                <Link to="/auth?mode=signup">
                  <Button size="sm" className="text-[13px] h-9 rounded-lg px-5 font-medium">
          ...
          {/* Mobile toggle */}
          <button onClick={() => setMobileOpen(!mobileOpen)} className="md:hidden p-2 rounded-lg hover:bg-muted/40 transition-colors">
      ...
      {mobileOpen && (
        <div className="md:hidden border-t border-border bg-background/98 backdrop-blur-2xl">
          <div className="px-6 py-5 space-y-1">
            ...
            <div className="pt-5 flex flex-col gap-2.5 border-t border-border mt-3">
              ...
              {user ? (
                <>
                  <Link to="/tracker" onClick={() => setMobileOpen(false)}>
                    <Button variant="outline" className="w-full rounded-lg gap-2 h-10 text-[13px]">
                      <User className="w-4 h-4" /> Dashboard
                    </Button>
                  </Link>
                  <Button variant="ghost" className="w-full rounded-lg gap-2 h-10 text-[13px]" onClick={() => { handleSignOut(); setMobileOpen(false); }}>
                ...
                <>
                  <Link to="/auth" onClick={() => setMobileOpen(false)}>
                    <Button variant="outline" className="w-full rounded-lg h-10 text-[13px]">Sign In</Button>
                  </Link>
                  <Link to="/auth?mode=signup" onClick={() => setMobileOpen(false)}>
                    <Button className="w-full rounded-lg h-10 text-[13px]">Get Started</Button>
                  </Link>
                </>
              )}
            </div>
