...
export function useHealthTwin() {
  ...
  const metrics = useMemo<HealthMetrics>(() => {
    ...
    const caloriesConsumed = todayFood.reduce((s: number, f: any) => s + (f.calories ?? 0), 0);
    const proteinConsumed = todayFood.reduce((s: number, f: any) => s + (f.protein_g ?? 0), 0);
    const carbsConsumed = todayFood.reduce((s: number, f: any) => s + (f.carbs_g ?? 0), 0);
    const fatConsumed = todayFood.reduce((s: number, f: any) => s + (f.fat_g ?? 0), 0);
    const fiberConsumed = todayFood.reduce((s: number, f: any) => s + (f.fiber_g ?? 0), 0);
    const waterProgress = waterLogs.reduce((s: number, w: any) => s + (w.glasses ?? 0), 0);

    // Today's workouts
    const todayWorkoutsRaw = workoutLogs.filter((w: any) => w.logged_at?.startsWith(today));
    const workoutsBurned = todayWorkoutsRaw.reduce((s: number, w: any) => s + (w.calories_burned ?? 0), 0);
    const workoutDuration = todayWorkoutsRaw.reduce((s: number, w: any) => s + (w.duration_min ?? 0), 0);
    const netCalories = caloriesConsumed - workoutsBurned;

    // Metabolism estimate (Mifflin-St Jeor)
    const wt = p?.weight_kg ?? 70;
    const ht = p?.height_cm ?? 170;
    const age = p?.age ?? 30;
    ...
    const weightTrend = [...weightLogs].reverse().map((w: any) => ({
    ...
    const todayWorkouts = todayWorkoutsRaw.map((w: any) => ({
