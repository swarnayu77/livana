...
const HealthTwin = () => {
  ...
  // Generate AI insights once metrics are loaded
  useEffect(() => {
  ...
  const generateInsights = async () => {
    ...
    try {
      ...
      setAiInsights([
        ...
        { text: metrics.waterProgress < metrics.waterTarget ? "Drink more water to boost energy and metabolism." : "Great hydration today! Keep it up.", type: metrics.waterProgress < metrics.waterTarget ? "warning" : "positive", icon: "droplets" },
      ]);
    } finally {
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="glass rounded-3xl p-6 md:p-8 relative overflow-hidden">
          ...
          <div className="flex flex-col md:flex-row items-center gap-8">
            ...
            <div className="flex-1 grid grid-cols-3 gap-4">
              ...
              <div className="flex flex-col items-center gap-1.5 text-center">
                <HealthScoreRing
                  score={metrics.energyLevel}
                  size={80}
                  strokeWidth={6}
                  label={`${metrics.energyLevel}`}
                  color="hsl(45 100% 50%)"
                />
              ...
              <div className="flex flex-col items-center gap-1.5 text-center">
                <HealthScoreRing
                  score={Math.min((metrics.waterProgress / metrics.waterTarget) * 100, 100)}
                  size={80}
                  strokeWidth={6}
                  label={`${metrics.waterProgress}`}
                  color="hsl(200 80% 55%)"
                />
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <MetricCard
            ...
            subtitle={`of ${metrics.calorieTarget} kcal`}
            accent
            trend={metrics.surplus ? "up" : "down"}
            trendValue={`${Math.abs(metrics.calorieBalance)} kcal`}
          />
          <MetricCard
            icon={Activity}
            label="Metabolism"
            value={`${metrics.metabolismRate}`}
            subtitle="kcal/day (TDEE)"
          />
          <MetricCard
            icon={Scale}
            label="Weight Trend"
            value={metrics.weeklyWeightChange !== 0 ? `${metrics.weeklyWeightChange > 0 ? "+" : ""}${metrics.weeklyWeightChange.toFixed(1)} kg` : "—"}
            subtitle="This period"
            trend={metrics.weeklyWeightChange > 0 ? "up" : metrics.weeklyWeightChange < 0 ? "down" : "neutral"}
            trendValue="weekly"
          />
          <MetricCard
            icon={Droplets}
            label="Water"
            value={`${metrics.waterProgress}/${metrics.waterTarget}`}
